...
void updatePinocchio(pinocchio::Model* const model,
                     pinocchio::Data* data,
...
BOOST_AUTO_TEST_CASE(test_partial_derivatives_against_numdiff) {
  ...
  boost::shared_ptr<crocoddyl::StateMultibody> state =
      boost::make_shared<crocoddyl::StateMultibody>(boost::make_shared<pinocchio::Model>(pin_model));
  ...
  boost::shared_ptr<qrw::ResidualModelFlyHigh> flyhigh_cost_model =
      boost::make_shared<qrw::ResidualModelFlyHigh>(state, id, 0.1, actuation_model->get_nu());
  ...
  for (int i = 0; i < 2; i++) {
    ...
    reevals.push_back(
        boost::bind(&updatePinocchio, &pin_model, &pin_data, boost::placeholders::_1, boost::placeholders::_2));
    reevals.push_back(boost::bind(
        &updateActuation, actuation_model, actuation_data, boost::placeholders::_1, boost::placeholders::_2));
