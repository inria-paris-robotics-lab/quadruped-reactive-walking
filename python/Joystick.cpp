...
struct JoystickVisitor : public bp::def_visitor<JoystickVisitor<Joystick>> {
  ...
  void visit(PyClassJoystick& cl) const {
    cl.def(bp::init<>(bp::args("self"), "Default constructor."))
        ...
        .def("initialize", &Joystick::initialize, bp::args("self", "params"), "Initialize Joystick from Python.\n")

        .def("update_v_ref", &Joystick::update_v_ref, bp::args("self", "k", "gait_is_static"), "Update joystick values.")

        .def("get_p_ref", &Joystick::getPRef, bp::args("self"), "Get Reference Position")
        .def("get_v_ref", &Joystick::getVRef, bp::args("self"), "Get Reference Velocity")
